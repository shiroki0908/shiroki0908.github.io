...
function initializenekko() {
    ...
    function daysLeft(dateStr, isLunar) {
        ...
        let nekkoDate;
        if (isLunar) {
            nekkoDate = LunarDate(now.getFullYear(), Month, Day);
            if (nekkoDate < now) {
                nekkoDate = LunarDate(now.getFullYear() + 1, Month, Day);
            }
        } else {
            nekkoDate = new Date(now.getFullYear(), Month - 1, Day);
            if (nekkoDate < now) {
                nekkoDate = new Date(now.getFullYear() + 1, Month - 1, Day);
            }
        }
        return daysBetween(now, nekkoDate);
    }
    // 经过天数
    ...
    function targetDate(dateStr, isLunar) {
        ...
        // 获取星期几
        const weekDay = nekkoDate.toLocaleDateString('zh-CN', { weekday: 'long' });
        // 返回年月日加星期几
        const year = nekkoDate.getFullYear();
        const month = (nekkoDate.getMonth() + 1).toString().padStart(2, '0'); // 月份从0开始，需要加1
        const day = nekkoDate.getDate().toString().padStart(2, '0');
        return `${year}-${month}-${day} (${weekDay})`; // 使用'-'作为分隔符
        //   return nekkoDate.toDateString();  // 直接返回斜杆日期
        // return nekkoDate.toLocaleDateString('zh-CN');
    }
...
// 初始页面加载
document.addEventListener("DOMContentLoaded", initializenekko);

// 适配 pjax
document.addEventListener("pjax:complete", initializenekko); 